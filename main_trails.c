...
//*************************************
    ...
    for(int i = 0; i < NUM_MENG; i++)
    {
        const float fi = (float)i;
        mIdent(&view);
        mTranslate(&view, 0.f, 0.f, zoom);
        mRotate(&view, yrot+(fi*0.1f), 1.f, 0.f, 0.f);
        mRotate(&view, xrot, 0.f, 0.f, 1.f);
        ...
        if(focus_cursor == 0 && dotick == 1)
        {
            ...
            g += urandfc()*dt*0.1f;
            b += urandfc()*dt*0.1f;
            r += urandfc()*(fi*0.0003f);
            g += urandfc()*(fi*0.0003f);
            b += urandfc()*(fi*0.0003f);
            r = clamp(r, -1.f, 1.f);
            g = clamp(g, -1.f, 1.f);
            b = clamp(b, -1.f, 1.f);
            glUniform3f(color_id, r, g, b);
        ...
        glUniform1f(opacity_id, 0.5f-(fi*0.01f));
        glUniformMatrix4fv(modelview_id, 1, GL_FALSE, (GLfloat*) &view.m[0][0]);
